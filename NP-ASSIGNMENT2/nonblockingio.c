/*
	T Dinesh Ram Kumar
	2014A3A7302P
	This program sends data to server using nonblocking I/O and select
...
int main(int argc, char *argv[]) {
	...
	while(1) {
		...
		if ( FD_ISSET(sockfd, &readset) ) {
			try ( readbytes = recv(sockfd, readbuffer + readfrom, BUFSIZ - readfrom, MSG_DONTWAIT), "read1 failed." );
			if ( readbytes == 0 ) {
			...
			readfrom = ( readfrom + readbytes )% BUFSIZ;
		}
		if ( FD_ISSET(sockfd, &writeset) ) {
			if ( writeto == 0 ) totalrequests ++;
			try ( writebytes = send(sockfd, buffer + writeto, BUFSIZ - writeto, MSG_DONTWAIT), "write1 failed." );
			totalwritebytes += writebytes;
			writeto = ( writeto +  writebytes)% BUFSIZ;
		}
